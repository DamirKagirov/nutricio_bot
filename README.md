...
Чтобы добавить скриншоты в `README.md` и ограничить их ширину до **400 пикселей**, используйте HTML-тег `<img>`:

```html
<img src="screenshots/1-personal-info.png" alt="Ввод антропометрических данных" width="400" />

<img src="screenshots/2-calories-result.png" alt="Результат расчёта калорий по формуле Миффлина-Сан Жеора" width="400" />

<img src="screenshots/3-week-menu.png" alt="Генерация недельного меню с ИИ" width="400" />
```
...
Git не сохраняет пустые папки, поэтому нужно создать файл внутри:

1. Создайте папку `screenshots/`, добавив файл через GitHub:  
   - Нажмите **"Add file" → "Create new file"**
   - Введите имя: `screenshots/.gitkeep` (или сразу `screenshots/example.png`)
2. Загрузите скриншоты:  
   - **"Add file" → "Upload files"** → выберите изображения
3. Используйте в `README.md`:

```html
<img src="screenshots/ваш_файл.png" alt="Описание" width="400" />
```

> ✅ `.gitkeep` — это пустой файл-заглушка, позволяющий Git отслеживать папку.
...
Есть идеи? Хотите протестировать? Пишите:  
📧 email@example.com  
